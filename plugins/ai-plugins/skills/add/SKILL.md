...
- Pins that SHA and prints which release or branch it came from. When the manifest is at the repo root, it writes a `url` source; when it is nested, a `git-subdir` source with that `path`. The Claude and Codex paths are detected independently, because they can differ within one repo.
