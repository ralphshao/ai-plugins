...
From the repo root, run `${CLAUDE_PLUGIN_ROOT}/skills/update/scripts/update.sh` (on Windows, `${CLAUDE_PLUGIN_ROOT}/skills/update/scripts/update.ps1`). Both are thin wrappers around `${CLAUDE_PLUGIN_ROOT}/scripts/ai-plugins.py update`.

What the script does:
- Reads every plugin in `.claude-plugin/marketplace.json` whose `source` is a `url`, `github`, or `git-subdir` object. It skips local entries such as `ai-plugins` itself.
