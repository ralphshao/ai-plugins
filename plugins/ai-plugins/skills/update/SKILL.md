...
- Uses `git ls-remote` to find each plugin's target commit. When the entry has a `ref`, that is the ref's latest commit. Otherwise it is the latest release (the highest `vX.Y.Z` tag, skipping pre-releases like `v2.0.0-beta`), or the default branch's latest commit when the repo has no release tags. A plugin pinned to a branch commit newer than its latest release moves back to the release.
